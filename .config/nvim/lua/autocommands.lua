...
-- [[ Recompile dwm on write ]]
local recompile_dwm = augroup("RecompileDWM", { clear = true })
local dwm_path = fn.resolve(fn.expand("~/.local/src/dwm-flexipatch/config.def.h"))
autocmd({ "BufWritePost" }, {
	pattern = dwm_path,
	callback = function()
		cmd([[!rm -f config.h patches.h && sudo make clean install && renew-dwm; notify-send " - dwm recompiled"]])
	end,
	group = recompile_dwm,
})
